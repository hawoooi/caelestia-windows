...
to the bottom of the strip). `bar.js` reads `bar.config.json`'s `entries` array, calls
`create(type, ctx)` for each name in order, appends each `.el` to `#bar`, and on every provider
tick calls every instance's `update(providerOutputMap)` inside a try/catch (one entry throwing
...
5. Write Node tests for the pure logic function(s) in `tests/js/entries.test.mjs`
   (`node --test`, not `node --test tests/js/` — see the mechanics note below).
...
restart per successful apply. The restart unconditionally kills and relaunches every running
`zebar.exe` process (see "no per-widget reload verb" above) — this is by design, not a bug to fix
later, given no alternative exists.
...
Install-Config -DryRun            # prints what would be patched/junctioned, touches nothing
Install-Config                    # patches ~/.config/whkdrc's caelestia-shell block, creates the junction
Install-Config -Uninstall         # removes the patched whkdrc block and the junction; repo untouched
```
