...
// ------------------------------------------------------- the window frame --
// THE FRAMELESS WINDOW WAS BUILT AND THEN REVERTED, on the user's call: the
// dragging was too fiddly to live with. A caption area behaves as caption, so
// anything inside it drags the window instead of reaching the panel -- which
// meant the drag region had to be the 8px rim above the bar and nothing more,
// and an 8px target that komorebi is simultaneously trying to tile is not a
// usable way to move a window.
//
// It DID work: foo_ui_wizard 0.2.8 is still installed at
// profile/user-components-x64/foo_ui_wizard/, and Preferences > Display >
// UI Wizard > Frame is set back to "Default". Changing that one dropdown to
// "No Caption" brings the frameless window back, and the window-control card
// this file used to draw is recoverable from git (commit c168bda).
//
// The one thing worth remembering if it is ever revived: the COM object must be
// acquired LAZILY. UI Wizard does not register its ProgID -- it HOOKS
// CLSIDFromProgID -- so a single `new ActiveXObject('UIWizard')` at panel load
...
function on_size() { TW = window.Width; TH = window.Height; _layout = null; }
